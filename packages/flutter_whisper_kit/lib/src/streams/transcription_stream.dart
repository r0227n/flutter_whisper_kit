...
class TranscriptionStream implements IDisposable {
  ...
  TranscriptionStream() : _controller = StreamController<TranscriptionEvent>.broadcast();
  ...
  void _handleOverflow(TranscriptionEvent event) {
    switch (overflowStrategy) {
      ...
      case OverflowStrategy.backpressure:
        // バックプレッシャーを適用（実装では何もしない）
        // プロダクションではログ出力ライブラリやコールバック機能を提供
        // 例: onOverflow?.call('Buffer overflow, applying backpressure');
        break;
    }
  }
