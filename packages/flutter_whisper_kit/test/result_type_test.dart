...
class TestException implements Exception {
  ...
  @override
  String toString() => code != null ? 'TestException($code): $message' : 'TestException: $message';
  ...
  group('Result', () {
    group('Success', () {
      ...
      test('should not map error on success', () {
        ...
        final mapped = result.mapError((exception) => TestException(exception.toString()));
    ...
    group('Failure', () {
      ...
      test('should return error with when pattern matching', () {
        final result = Failure<String, TestException>(TestException('Not found', 404));
      ...
      test('should map error on failure', () {
        final result = Failure<String, TestException>(TestException('Not found', 404));
        final mapped = result.mapError((exception) => TestException('Error code: ${exception.code}'));
    ...
    group('Complex type scenarios', () {
      test('should work with nullable types', () {
        ...
        final failureResult = Failure<String?, TestException>(TestException('error', 0));
        
        expect(successResult.isSuccess, isTrue);
        expect(successResult.when(
          success: (value) => value,
          failure: (_) => 'error',
        ), isNull);
        
        expect(failureResult.isFailure, isTrue);
        expect(failureResult.when(
          success: (_) => 'success',
      ...
      test('should chain error transformations', () {
        final result = Failure<int, TestException>(TestException('Not found', 404));
        
        final transformed = result
            .mapError((exception) => TestException('Error ${exception.code}'))
            .mapError((exception) => TestException('${exception.message} not found'))
            .mapError((exception) => TestException(exception.message.toUpperCase()));
      ...
      test('should preserve error type through value transformations', () {
        final result = Failure<int, TestException>(TestException('original error'));
